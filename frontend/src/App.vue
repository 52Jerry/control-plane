...
const defaultUserPolicy = ref(null)
...
async function loadAll() {
  const [, policy] = await Promise.all([
    loadNodes({ awaitDashboard: false }),
    api.defaultUserPolicy(),
  ])
  defaultUserPolicy.value = policy
...
async function exportNodeUsers() {
  ...
  try {
    ...
    const accelerationExportScenarios = new Set([
      'vless', 'socksAcceleration', 'vmess', 'fingerprintAcceleration',
    ])
    const includeAccelerationDomain = accelerationExportScenarios.has(scenario.key)
    ...
    const rows = exportUsers.map((item) => {
      let connectionData = {
        ip: '', accelerationDomain: '', port: '', username: '', password: '', link: '',
      }
    ...
    rows.forEach((rowData, rowIndex) => {
      const excelRow = worksheet.addRow({
        ...
        ip: rowData.ip,
        accelerationDomain: rowData.accelerationDomain,
        port: rowData.port,
    ...
    <div v-if="modal.provision" class="modal-backdrop" @mousedown.self="modal.provision = false">
      <form class="modal-card wide-card" @submit.prevent="provisionDirect">
        ...
        <div class="form-grid">
          ...
          <label>流量额度（GB）<input v-model.number="provisionForm.trafficLimitGb" type="number" min="0" step="0.1" placeholder="数据库默认值" /></label>
          <label>最大同时来源 IP 数<input v-model.number="provisionForm.maxSourceIps" type="number" min="0" max="1000" step="1" placeholder="数据库默认值" /></label>
        </div>
